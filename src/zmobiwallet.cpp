...
CzMOBIWallet::CzMOBIWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zmobi seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMOBISeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMOBISeed_deprecated()) {
                    LogPrintf("%s: Updated zMOBI seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zmobi seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zmobi wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzMOBIWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMOBICount(nCountLastUsed);
    else if (!walletdb.ReadZMOBICount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMOBIWallet::Lock()
{
    seedMaster = 0;
}

void CzMOBIWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMOBIWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMOBI(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMOBIWallet::LoadMintPoolFromDB()
{
...
void CzMOBIWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMOBIWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMOBIWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zmobiTracker->HasPubcoinHash(pMint.first)) {
...
bool CzMOBIWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMOBI(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zmobiTracker which also adds to database
    pwalletMain->zmobiTracker->Add(dMint, true);
...
void CzMOBIWallet::SeedToZMOBI(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMOBIWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMOBIWallet::GenerateDeterministicZMOBI(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMOBIWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMOBIWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
