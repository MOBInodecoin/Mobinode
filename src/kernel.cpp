...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZMobiStake(spend));
    } else {
        ...
        CMobiStake* mobiInput = new CMobiStake();
        mobiInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(mobiInput);
    }
