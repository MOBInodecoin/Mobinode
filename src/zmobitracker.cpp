...
CzMOBITracker::CzMOBITracker(std::string strWalletFile)
{
...
CzMOBITracker::~CzMOBITracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzMOBITracker::Init()
{
...
bool CzMOBITracker::Archive(CMintMeta& meta)
{
...
bool CzMOBITracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzMOBITracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzMOBITracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzMOBITracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzMOBITracker::GetSerialHashes()
{
...
CAmount CzMOBITracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzMOBITracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzMOBITracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzMOBITracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzMOBITracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzMOBITracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzMOBITracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzMOBITracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzMOBITracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzMOBITracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzMOBITracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMOBITracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMOBITracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzMOBITracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzMOBITracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzMOBITracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzMOBITracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzmobi from DB\n", __func__, listDeterministicDB.size());
    }
