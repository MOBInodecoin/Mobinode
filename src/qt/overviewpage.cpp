...
class TxViewDelegate : public QAbstractItemDelegate
{
	Q_OBJECT
public:
	TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::MOBI)
	{
	}
	...
	const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
	...
	CAmount mobiAvailableBalance = balance - immatureBalance - nLockedBalance;
	CAmount nUnlockedBalance = nTotalBalance - nLockedBalance;

	// Mobinode Watch-Only Balance
	CAmount nTotalWatchBalance = watchOnlyBalance + watchUnconfBalance;
	CAmount nAvailableWatchBalance = watchOnlyBalance - watchImmatureBalance - nWatchOnlyLockedBalance;

	// Combined balances
	CAmount availableTotalBalance = mobiAvailableBalance; //+ matureZerocoinBalance;
	CAmount sumTotalBalance = nTotalBalance; // + zerocoinBalance;

											 // MOBI labels
	ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, mobiAvailableBalance, false, BitcoinUnits::separatorAlways));
	...
	QString automintHelp = tr("Current percentage of zMOBI.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
	...
	// MOBI Available
	bool showMOBIAvailable = settingShowAllBalances || mobiAvailableBalance != nTotalBalance;
	bool showWatchOnlyMOBIAvailable = showMOBIAvailable || nAvailableWatchBalance != nTotalWatchBalance;
	ui->labelBalanceText->setVisible(showMOBIAvailable || showWatchOnlyMOBIAvailable);
	ui->labelBalance->setVisible(showMOBIAvailable || showWatchOnlyMOBIAvailable);
	ui->labelWatchAvailable->setVisible(showWatchOnlyMOBIAvailable && showWatchOnly);

	// MOBI Pending
	bool showMOBIPending = settingShowAllBalances || unconfirmedBalance != 0;
	bool showWatchOnlyMOBIPending = showMOBIPending || watchUnconfBalance != 0;
	ui->labelPendingText->setVisible(showMOBIPending || showWatchOnlyMOBIPending);
	ui->labelUnconfirmed->setVisible(showMOBIPending || showWatchOnlyMOBIPending);
	ui->labelWatchPending->setVisible(showWatchOnlyMOBIPending && showWatchOnly);

	// MOBI Immature
	bool showMOBIImmature = settingShowAllBalances || immatureBalance != 0;
	bool showWatchOnlyImmature = showMOBIImmature || watchImmatureBalance != 0;
	ui->labelImmatureText->setVisible(showMOBIImmature || showWatchOnlyImmature);
	ui->labelImmature->setVisible(showMOBIImmature || showWatchOnlyImmature); // for symmetry reasons also show immature label when the watch-only one is shown
	ui->labelWatchImmature->setVisible(showWatchOnlyImmature && showWatchOnly); // show watch-only immature balance

																				// MOBI Locked
	bool showMOBILocked = settingShowAllBalances || nLockedBalance != 0;
	bool showWatchOnlyMOBILocked = showMOBILocked || nWatchOnlyLockedBalance != 0;
	ui->labelLockedBalanceText->setVisible(showMOBILocked || showWatchOnlyMOBILocked);
	ui->labelLockedBalance->setVisible(showMOBILocked || showWatchOnlyMOBILocked);
	ui->labelWatchLocked->setVisible(showWatchOnlyMOBILocked && showWatchOnly);
...
void OverviewPage::updatBlockChainInfo()
{
	if (masternodeSync.IsBlockchainSynced())
	{
		...
		ui->label_Supply_value->setText(QString::number(chainActive.Tip()->nMoneySupply / COIN).append(" MOBI"));
