...
#include "ui_zmobicontroldialog.h"
...
std::set<std::string> ZMobiControlDialog::setSelectedMints;
std::set<CMintMeta> ZMobiControlDialog::setMints;

ZMobiControlDialog::ZMobiControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZMobiControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZMobiControlDialog::~ZMobiControlDialog()
{
    delete ui;
}

void ZMobiControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMobiControlDialog::updateList()
{
...
void ZMobiControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMobiControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZMobi_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMobiControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZMobiControlDialog::GetSelectedMints()
{
...
void ZMobiControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
