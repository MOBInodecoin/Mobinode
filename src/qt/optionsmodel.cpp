...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MOBI);
    ...
    if (!settings.contains("nAnonymizeMobinodeAmount"))
        settings.setValue("nAnonymizeMobinodeAmount", 1000);

    nAnonymizeMobinodeAmount = settings.value("nAnonymizeMobinodeAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMobinodeAmount"))
        SoftSetArg("-anonymizemobinodeamount", settings.value("nAnonymizeMobinodeAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in mobinode.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMobinodeAmount:
            return QVariant(nAnonymizeMobinodeAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMobinodeAmount:
            nAnonymizeMobinodeAmount = value.toInt();
            settings.setValue("nAnonymizeMobinodeAmount", nAnonymizeMobinodeAmount);
            emit anonymizeMobinodeAmountChanged(nAnonymizeMobinodeAmount);
            break;
        case CoinControlFeatures:
